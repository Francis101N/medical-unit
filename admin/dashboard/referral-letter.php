...
$current_record_id = 0; // Initialize record ID fallback
...
if (isset($_GET['ref_id'])) {
    ...
    if ($id) {
        $current_record_id = $id; // <-- Assigned decrypted ID here so JS receives it properly
...
<body>
    ...
    <!-- JavaScript Live Binding & PDF Generation Logic -->
    <script>
        const currentRecordId = <?php echo json_encode($current_record_id); ?>;
        ...
        const fields = {
            ...
            staffAge: document.getElementById('staffAge'),
        ...
        // Real-time input synchronization binding
        Object.keys(fields).forEach(key => {
            if (fields[key] && previews[key]) {
                fields[key].addEventListener('input', () => {
                    previews[key].textContent = fields[key].value || '\u00a0';
                });
            }
        });
        ...
        document.getElementById('logoSelect').addEventListener('change', function() {
            const logoSrc = this.value;
            const logoImg = document.getElementById('logoImageDisplay');
        ...
        document.getElementById('signatureInput').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(event) {
                    const sigImg = document.getElementById('signatureImageDisplay');
                    ...
                    sigPlaceholder.classList.add('d-none');
                }
                reader.readAsDataURL(file);
            }
        });

        // Direct PDF Download Button Handler (Saves to server folder & triggers download)
        document.getElementById('printBtn').addEventListener('click', function() {
            const printBtn = document.getElementById('printBtn');
            printBtn.disabled = true;
            printBtn.textContent = 'Generating & Saving PDF...';

            const element = document.getElementById('printableLetter');
            const pdfFilename = `Medical_Referral_${fields.refCode.value}.pdf`;
            ...
            html2pdf().from(element).set(opt).outputPdf('blob').then(function(pdfBlob) {
                ...
                .then(data => {
                    ...
                    // Automatically trigger the client-side download right after saving server-side
                ...
                .catch(err => {
                    ...
                    printBtn.disabled = false;
                    printBtn.textContent = 'Print / Save as PDF';
                    alert('An error occurred while generating or saving the PDF.');
                });
        });
        ...
        document.getElementById('downloadTxtBtn').addEventListener('click', function() {
            let textContent = "========================================\r\n";
            textContent += "        MEDICAL REFERRAL NOTE           \r\n";
            textContent += "========================================\r\n\r\n";
            textContent += `Ref Code: ${fields.refCode.value}\r\n`;
            textContent += `Note No: ${fields.noteNo.value}\r\n`;
            textContent += `Date: ${fields.formDate.value} ${fields.formTime.value}\r\n\r\n`;
            textContent += `Staff Name: ${fields.staffName.value}\r\n`;
            textContent += `Department: ${fields.department.value}\r\n`;
            textContent += `Sex: ${fields.staffSex.value} | Age: ${fields.staffAge.value}\r\n\r\n`;
            textContent += `--- VITAL SIGNS ---\r\n`;
            textContent += `Temp: ${fields.vitalTemp.value} | Pulse: ${fields.vitalPulse.value} | Res: ${fields.vitalRes.value} | BP/SpO2: ${fields.vitalBpSpo.value}\r\n\r\n`;
            textContent += `Client Complaint: ${fields.clientComplaint.value}\r\n`;
            textContent += `Duration: ${fields.complaintDuration.value} | Medical Unit: ${fields.medicalUnit.value}\r\n`;
            textContent += `Previous Medication: ${fields.prevMedication.value}\r\n`;
            textContent += `Investigation Done: ${fields.investigationDone.value} (${fields.investigationWhat.value})\r\n\r\n`;
            textContent += `Reason for Referral:\r\n${fields.reasonForReferral.value}\r\n\r\n`;
            textContent += `Referring Officer: ${fields.referringOfficer.value} (${fields.officerContact.value})\r\n`;
            textContent += "========================================\r\n";
            ...
            link.download = `Medical_Referral_${fields.refCode.value}.txt`;
            link.click();
